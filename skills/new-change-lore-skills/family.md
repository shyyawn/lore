...
| `svelte` | platform | `typescript-idioms` + Svelte plugin | Pin, `$app/state`. Not a fork of the plugin |
...
| `next` in `package.json` | Project `AGENTS.md` + bundled `next` docs. Workflows: official `vercel/next.js` skills. Performance: `vercel-react-best-practices`. Tests: `typescript-unit-tests` + pin Testing guide; journeys: `e2e-tests`. Do not fork retired `next-skills` |
| web `react` without `next` or `expo` | Official `vercel-react-best-practices`. Tests: `typescript-unit-tests`. Do not flatten a Vite `src/` into Next `app/` |
...
2. **Named starters / case studies**, then say what **not** to copy
   ...
   | Svelte `kit/project-structure`, `sv create` | `src/routes`, `$lib`, `$lib/server` | Plugin runes catalog |
