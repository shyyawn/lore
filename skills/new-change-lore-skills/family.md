...
| `next` in `package.json` | Project `AGENTS.md` + bundled `next` docs. Workflows: official `vercel/next.js` skills. Performance: `vercel-react-best-practices`. Look: `frontend-design`. Audit: `web-design-guidelines`. Tests: `typescript-unit-tests` + pin Testing guide; journeys: `e2e-tests`. Do not fork retired `next-skills` |
| web `react` without `next` or `expo` | Official `vercel-react-best-practices`. Look: `frontend-design`. Audit: `web-design-guidelines`. Tests: `typescript-unit-tests`. Do not flatten a Vite `src/` into Next `app/` |
...
| Official `frontend-design` | Web look / anti-slop | A lore fork; Expo / RN screens (use `expo/skills`) |
| Official `vercel-react-best-practices` | Web React / Next performance | A lore dump of Vercel's rule files |
| Official `web-design-guidelines` | Web UI audit (a11y, spacing, focus) | `anti-ui-slop` clones; a lore HIG dump |
