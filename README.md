...
| [pi-extensions](skills/pi-extensions) | When a Pi change is AGENTS.md, a prompt, a skill, or a TypeScript extension; how to write the extension when earned |
...
| [source-of-truth](skills/source-of-truth) | One writer per fact; a GET hits one store; dual-write needs an outbox |
| [data-modeling](skills/data-modeling) | Logical schema: table vs blob vs junction; one fact one type |
| [identity](skills/identity) | Natural entity → business key; UUID is extra uniqueness |
...
| [requirement-to-architecture-to-design](skills/requirement-to-architecture-to-design) | Requirements then architecture then design; do not skip a stage |
| [review-change](skills/review-change) | Tick catalogs 1–7 plus the language pin; do not recopy them |
| [customize-cv](skills/customize-cv) | Career source `SOURCE.md` → IC or Manager job CV; short cover letter when asked; tailor when a JD exists, else a base |
| [customize-cv-freelance](skills/customize-cv-freelance) | Overlay on `customize-cv`: pass-along or marketplace listing |
| [customize-cv-linkedin](skills/customize-cv-linkedin) | Overlay on `customize-cv`: job LinkedIn. Default is no |
